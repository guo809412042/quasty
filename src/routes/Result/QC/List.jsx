...
import {
  SOURCE, IS_PASS, IS_PASS_COLOR, CHECK_TYPE, CHECK_COLOR, ISREPLY_COLOR, VIP_STATUS,
} from '../../const';
import { APP_PRODUCT_LIST } from '../../../utils/const';
import { formatDate, gotoIssuePage } from '../../../utils/utils';
import DetailView from '../customerService/components/DetailView';
...
export default ({
  ...
  columns = columns.concat([
    ...
    },
    {
      ...
      title: '是否合格',
      render: text => <Tag color={IS_PASS_COLOR[text]}>{IS_PASS[text]}</Tag>,
    },
    {
      ...
      render: (text, row) => (
        <div>
          <RateView
            row={row}
            getList={getList}
            source={row.qualityCheck.source || source}
            scoreConfigData={scoreConfigData}
          />
          <DetailView row={row.qualityStatus} getList={getList} />
        </div>
      ),
    },
  ]);
